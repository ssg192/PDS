...
SELECT (CASE WHEN random() < 0.5 THEN 'Dr. ' ELSE 'Dra. ' END) ||
       (ARRAY[
            'Juan', 'José', 'Luis', 'Carlos', 'Miguel', 'Francisco', 'Jesús', 'Javier', 'David', 'Alejandro',
        'Fernando', 'Manuel', 'Arturo', 'Ricardo', 'Pablo', 'Sergio', 'Diego', 'Roberto', 'Alberto', 'Raúl'
            ])[floor(random()*20)::int+1] || ' ' ||
  (ARRAY[
    'García','Hernández','Martínez','López','González','Pérez','Sánchez','Ramírez','Torres','Flores',
  ...
  CASE WHEN random()<0.8 THEN 'Activo' ELSE 'Inactivo'
...
SELECT (ARRAY[
            'María', 'Juana', 'Ana', 'Laura', 'Carmen', 'José', 'Luis', 'Carlos', 'Antonio', 'Francisco',
  ...
  current_date - (random()*3650)::int               AS fecha_afiliacion,  -- últimos 10 años
  floor(random()*3)::int+1                          AS id_estado        -- 1–3 (Activo/Inactivo/Pendiente)
FROM generate_series(1, 1000) AS s(i);

-- 7) Hechos Cita (5 000 filas)
INSERT INTO th07_cita
(fk_id_fecha, fk_id_hospital, fk_id_medico, fk_id_paciente, fk_id_estado)
SELECT floor(random() * 365)::int + 1,    -- fechas 1–365 floor(random() * 20)::int + 1,     -- hospitales 1–20 floor(random() * 200)::int + 1,    -- médicos 1–200 floor(random() * 1000)::int + 1,   -- pacientes 1–1000 floor(random() * 3)::int + 1       -- estados 1–3 (Programada/Atendida/Cancelada)
